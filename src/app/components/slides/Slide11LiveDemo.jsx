...
import { ScanFace, ExternalLink, Globe, Sparkles, QrCode, Smartphone } from "lucide-react";
...
export default function Slide11LiveDemo() {
  ...
  return (
    <motion.div
      className="flex flex-col items-center justify-between h-full text-center py-3 px-4 overflow-hidden"
      variants={container}
      initial="hidden"
      animate="visible"
    >
      ...
      <motion.div variants={item} className="w-full max-w-3xl glass-card p-5 border border-gold-400/40 rounded-2xl shadow-2xl relative overflow-hidden my-auto">
        ...
        <div className="flex flex-col items-center space-y-4 relative z-10">
          ...
          <div className="space-y-1">
            ...
            <p className="text-cream-300 text-xs max-w-xl mx-auto leading-relaxed">
              Bapak/Ibu Dosen Penguji dipersilakan mengakses domain produksi via smartphone/laptop untuk mensimulasikan pengujian langsung (kartu fisik e-KTP dummy dibagikan saat sidang):
            </p>
          </div>

          {/* Prominent Demo URL Box for Vercel Domain */}
          <div className="w-full max-w-xl bg-black/50 border border-gold-400/50 rounded-xl p-3 flex items-center justify-between gap-3 shadow-inner">
            <div className="flex items-center gap-2.5 px-2 overflow-hidden flex-1">
              <Globe size={20} className="text-cyan-400 flex-shrink-0" />
              <input
                type="text"
                value={demoUrl}
                onChange={(e) => setDemoUrl(e.target.value)}
                className="bg-transparent text-gold-300 font-mono text-sm font-bold w-full focus:outline-none tracking-wide"
              />
            </div>
            <a
              href={demoUrl}
          ...
          <div className="grid grid-cols-3 gap-3 w-full max-w-xl text-left pt-2">
            <div className="glass-card p-2.5 rounded-lg border border-white/10 text-center">
              ...
              <p className="text-[0.72rem] text-cream-200">Akses Domain Vercel &amp; Klik Scan KTP</p>
            </div>
            <div className="glass-card p-2.5 rounded-lg border border-white/10 text-center">
              <span className="text-[0.65rem] font-bold uppercase tracking-wider text-cyan-400 block mb-0.5">Langkah 2</span>
              <p className="text-[0.72rem] text-cream-200">Posisikan KTP Dummy pada Kamera Sketsa</p>
            </div>
            <div className="glass-card p-2.5 rounded-lg border border-white/10 text-center">
              <span className="text-[0.65rem] font-bold uppercase tracking-wider text-emerald-400 block mb-0.5">Langkah 3</span>
              <p className="text-[0.72rem] text-cream-200">Formulir Terisi Otomatis (32,77 Detik)</p>
            </div>
          </div>
        </div>
      </motion.div>
